      ...
      <div className="mx-auto bg-gray-100 px-4 sm:px-6 lg:px-8">
        <div className="bg-white shadow">
          <div className="p-6 sm:p-12">
            ...
            <div className="mt-6">
              <h1 className="font-bold text-lg">Вашите адреси:</h1>
              {pageLevelLoader ? (
                <PulseLoader
                  color={"#000000"}
                  ...
                  {addresses && addresses.length ? (
                    addresses.map((item) => (
                      <div className="border p-6" key={item._id}>
                        <p>Име : {item.fullName}</p>
                        <p>Адрес : {item.address}</p>
                        <p>Град : {item.city}</p>
                        <p>Държава : {item.country}</p>
                        <p>Пощенски код : {item.postalCode}</p>
                        <button
                    ...
                    <p>Все още нямате добавен адрес.</p>
                  )}
                </div>
              )}
            </div>
            <div className="mt-4">
              <button
                ...
                className="mt-5  inline-block bg-black text-white px-5 py-3 text-xs font-medium uppercase tracking-wide"
              >
                {showAddressForm ? "Скрий полето за адреси" : "Добавяне на нов адрес"}
              </button>
            </div>
            {showAddressForm ? (
